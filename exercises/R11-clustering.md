...
Create an R script that contains the following code:
```
# preliminaries -----------------------------------
library(tidyverse)
library(fixest)
...
for (i in 1:loopmax) {
  ...
  data <- tibble(
    clustid = 1:numclusters,
    clusteffect = rnorm(numclusters)
  )
  data <- data %>% 
    mutate(treatment = if_else(clustid >= numclusters/2, 1, 0)) %>%
    slice(rep(1:numclusters, each = obspercluster)) %>% 
    mutate(epsilon = rnorm(numclusters * obspercluster), 
           y = clusteffect + effect * treatment +epsilon)
  ols <- feols(y ~ treatment, data = data)
  pvals[i] <- ols$coeftable[2,4]
}

significant = if_else(pvals < 0.05, 1, 0)
mean(significant)
```
...
where you can store results.  We can use this to 
...
zero and variance one, and an observation-specific error term, which is also a standard normal - plus a treatment effect which 
...
```
slice(data, rep(1:numclusters, each = obspercluster))
```
...
sized unless we cluster the standard errors.  Run your code again after replacing the regression 
specification with:
```
ols <- feols(y ~ treatment, data = data, vcov = ~clustid)
```
...
You can install the `lmer` and `performance` packages use the code below to estimate the degree of intraclass correlation, &rho;, and store your estimates (in an empty vector 
`rho` that you create).  Add this code to your loop after you store the results from the main regression of `y` on `treatment`:
```
rho_model <- lmer(y ~ 1 + (1 | clustid), data = data)
rho[i] <- icc(rho_model)$ICC_adjusted
```

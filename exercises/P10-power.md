# Empirical Exercise 10 in Python
...
data from [Price Subsidies, Diagnostic Tests, and Targeting of Malaria Treatment: Evidence from a Randomized Controlled Trial](https://www.aeaweb.org/articles?id=10.1257/aer.20130267) by 
Jessica Cohen, Pascaline Dupas, and Simone Schaner.  We will also use data from 
the paper [The Miracle of Microfinance?  Evidence from a Randomized Evaluation](https://www.jstor.org/stable/43189512?seq=1) by 
Abhijit Banerjee, Esther Duflo, Rachel Glennerster, and Cynthia Kinnan.  We used 
[this latter data](https://pjakiela.github.io/ECON379/exercises/E6-BanerjeeEtAl-data.dta) set in Empirical Exercises 6 and 9.
...
Write code that calculates the estimated **standard deviation** of 
`b_knowledge_correct` and the number of non-missing observations.  Write an additional line of code to display the MDE given the standard deviation of 
...
Hint: to find the number of non-missing observations of variable `varname` in data frame `df`, you can use the code:
```
df['varname'].count().sum()
```

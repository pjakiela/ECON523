...
trials.  Researchers often assign treatment at the cluster level (e.g. at the school, community, or market level) when we are worried about 
...
saves the results of a regression in the matrix `V`, as we've seen before.  For example, if you ran the command 
`reg y x1 x2` and saw the Stata output below.  

![reg-output](https://pjakiela.github.io/ECON523/exercises/reg-output.png)

If you ran the command `mat V = r(table)` and then used the command `mat list V` 
to display the matrix, you'd see this:  

![mat-list](https://pjakiela.github.io/ECON523/exercises/mat-list.png)
...
MDE (of 0.25)?  Confirm this by updating your sample size accordingly.
...
An intra-class correlation of 0.49 is **very** high.  Even students within the same classroom in the same school 
have to an intra-class correlation (in test scores) closer to 0.1 or 0.2.  How large of a sample would you need 
to be powered to detect an impact of 0.25 if the intra-class correlation were 0.2 instead of 0.49?

<br>

#### Congratulations!

You finished all the empirical exercises in ECON 523!  Hurray!  And now you know how to calculate 
the required sample size for an individually-randomized or a cluster-randomized study.  As you've seen, cluster-randomized trials 
